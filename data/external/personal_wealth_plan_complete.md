# Personal Wealth Plan: Complete Financial Assessment

**Date:** September 21, 2026  
**Updated from:** Banktivity Export  
**Time Horizon:** 20–30+ years  
**Primary Goal:** Maximize lifetime income; build $500k estate

---

## CURRENT FINANCIAL POSITION

### Net Worth Summary

| Category | Balance | % of Assets |
|----------|---------|-------------|
| **Checking/Savings** | $309,908 | 54.5% |
| **Investments** | $39,847 | 7.0% |
| **Other Assets** | $218,206 | 38.4% |
| **Credit Cards** | ($9,967) | (1.8%) |
| **NET WORTH** | **$557,994** | **100%** |

**Key Observations:**
- Net worth is $557,994—exceeds $500k estate goal
- High liquid savings ($309,908 in cash/checking): 41 months of expenses
- Real estate holdings: $125,000 (Hawthorne home) + $93,206 (Aircraft loan balance—unclear if asset or debt)
- Investments are minimal: only $39,847 across 7 accounts (mostly retirement)
- Credit card debt is trivial: $9,967 total across 4 cards

---

## INCOME ANALYSIS

### Monthly Guaranteed Income
- **Social Security:** $3,100/month (inflation-adjusted)
- **VA Disability (MS):** $4,388/month (inflation-adjusted)
- **Total:** $7,488/month ($89,856/year, indexed)

**Status:** Fully secure, uncorrelated with market conditions.

---

## EXPENSE ANALYSIS

### Current Spending
- **Reported monthly spending:** ~$2,300 (from prior conversation; transaction-level breakdown pending)
- **Monthly coverage ratio:** $7,488 / $2,300 = 3.25x
- **Annual expense run rate:** ~$27,600
- **Excess cash flow:** $5,188/month available for investment
- **Healthcare cost:** **$0/month** — VA (primary coverage) + Medicare (secondary) = fully covered

### Projected Monthly Expense Budget

| Category | Est. Monthly |
|---|---|
| Housing & utilities (Hawthorne) | $700 |
| Food & groceries | $350 |
| Transportation | $250 |
| Disability supplies & personal care | $150 |
| Entertainment & subscriptions | $200 |
| Colombia-related expenses | $250 |
| Miscellaneous & buffer | $200 |
| **Healthcare** | **$0** |
| **Total** | **~$2,100–$2,300** |

Zero healthcare cost is a structural advantage worth ~$6,000–$18,000/year compared to peers with comparable health profiles. This amount remains fully investable.

### Reserve Position
- **Liquid emergency fund:** $309,908
- **Months of expenses covered:** 134 months (11+ years)
- **Assessment:** Massively over-reserved; excess funds should be deployed

---

## INVESTMENT POSITION ANALYSIS

### Current Holdings

| Account | Type | Balance | Status |
|---------|------|---------|--------|
| Individual Bond Ladder | Inv | $39,384 | Core holding; generating yield |
| Kenneth's Rollover IRA (FG) | Inv | $12.88 | Minimal |
| Kenneth's TODI IRA (FG) | Inv | $252 | Minimal |
| Kenneth's Trad IRA (FG) | Inv | $71.16 | Minimal |
| Resurety Stock | Inv | $0 | Abandoned/worthless |
| SEP-IRA | Inv | ($25.14) | Closed position |
| Wealthfront Joint Acct | Inv | $151 | Minimal |
| **Total Investments** | — | **$39,847** | **7.0% of assets** |

**Assessment:**
- Current allocation is heavily skewed toward cash (54.5% in checking)
- Only $39.4k in bonds (core holding); no equities
- Retirement accounts are severely underfunded
- No real estate leverage or systematic equity exposure

---

## RISK ASSESSMENT & CONSTRAINTS

### Age & Health Constraints
- **Age:** 64; wheelchair-bound with MS
- **Life expectancy:** 20–30+ years (MS typically does not reduce life expectancy significantly)
- **Healthcare:** Fully covered by VA (primary) and Medicare (secondary) — $0/month out-of-pocket
- **Care risk:** Potential future increase in personal care/aide costs (not medical)
- **Constraint:** Cannot be aggressive; need predictable income and liquidity

### Income Security
- **Social Security + VA Disability:** Fully indexed, non-market-correlated
- **Implication:** Portfolio can sustain equity exposure without sacrificing income certainty
- **Sequence risk:** Must manage withdrawal sequence carefully in early retirement

### Liquidity Needs
- **Colombia home:** $25,000 renovation (optional, timeline flexible)
- **Ongoing monthly:** $2,300 (covered by guaranteed income)
- **Implication:** $309k cash reserve is more than adequate; excess should be invested

---

## GOALS & TARGETS

### Primary Goal: $500k Estate
- **Current position:** $557,994 net worth
- **Status:** ALREADY ACHIEVED
- **But:** No growth trajectory without investment

### Secondary Goal: Lifetime Income Maximization
- **Strategy:** Deploy excess cash flow into diversified, income-generating portfolio
- **Expected outcome:** Growth to $750k–$1.2M by year 5–10

### Tertiary Goal: Healthcare & Care Support
- **Risk:** Future care needs could be significant
- **Mitigation:** Maintain $100k+ emergency reserve; invest remainder

---

## RECOMMENDED PORTFOLIO ALLOCATION

### Strategic Allocation (Long-Term)
This allocation balances income generation, growth, and volatility constraints:

| Asset Class | Allocation | $ Amount | Rationale |
|------------|-----------|----------|-----------|
| **U.S. Large Cap Equities** | 20% | $111,599 | Core holding; liquid, diversified |
| **International Developed** | 15% | $83,699 | Diversification; currency hedge |
| **Emerging Markets** | 15% | $83,699 | Growth potential; long horizon |
| **Bonds (Core)** | 25% | $139,498 | Income; stability; rollable |
| **Infrastructure (Income)** | 10% | $55,799 | Inflation-hedge; yield (4–5%) |
| **Commodities/Transition** | 5% | $27,900 | Diversification; inflation hedge |
| **Cash Reserve** | 10% | $55,799 | Emergency fund; rebalancing |
| **TOTAL** | 100% | $557,993 | — |

**Rationale:**
- Significant equity exposure (50%) due to long horizon and income security floor
- Bond ladder provides immediate yield and ballast
- Infrastructure and commodities provide inflation protection
- Cash reserve ensures stability and flexibility

---

## INVESTMENT VEHICLE RECOMMENDATIONS

### Platform Philosophy: Low-Overhead, Automated

The plan centers on **Wealthfront** as the primary investment platform, with the existing bond ladder retained as a separate income position. Wealthfront provides:

- **0.25% annual advisory fee** — among the lowest available for a full-service robo-advisor
- **Automated daily tax-loss harvesting** — captures losses systematically without manual effort
- **Automatic rebalancing** — no manual intervention needed
- **Automated investing** — deposits allocated and invested immediately

No individual ETF selection or manual rebalancing is required. You set a risk score and deposit; Wealthfront manages the rest.

### Accounts

| Account | Platform | Role | Current Balance |
|---|---|---|---|
| Individual Cash Account | Wealthfront | Emergency fund / HYSA | $153,492 |
| Joint Investment Account | Wealthfront | Core taxable portfolio | $151 (seed) |
| Individual Bond Ladder | Existing | Fixed income / income | $39,384 |
| IRA accounts (Fidelity) | Fidelity | Tax-advantaged (minimal) | ~$336 |

### Wealthfront Risk Score Guidance

Wealthfront uses a 0.5–10 risk score to determine equity/bond mix. Given:
- Guaranteed income floor ($7,488/month) removing sequence-of-returns risk
- Long 20–30+ year horizon
- Already-met estate goal providing downside cushion

**Recommended risk score: 7.5–8.5** (approximately 80–90% equities, 10–20% bonds/alternatives). Wealthfront will automatically include U.S. stocks, international stocks, emerging markets, bonds, real estate (REITs), and natural resources at this score — matching the target allocation without manual ETF selection.

### AI Company Exclusion Policy

You have explicitly stated you do not want financial exposure to major AI companies due to valuation concerns. The primary names of concern are NVIDIA (NVDA), Microsoft (MSFT), Alphabet (GOOGL/GOOG), Meta (META), and Amazon (AMZN).

**Implementation:** Enable **Wealthfront Direct Indexing** (available at your asset level — $100k+ minimum) and add stock exclusions for the five names above. Wealthfront will hold substitute positions to maintain factor exposure while excluding the named stocks.

This is the most practical solution within a low-overhead automated framework. International ETFs (VEA, VWO), bonds, infrastructure, and real assets carry near-zero exposure to these companies and naturally provide AI avoidance for those portions of the portfolio.

### Supplemental Positions (Outside Wealthfront)

| Position | Purpose | Vehicle |
|---|---|---|
| Existing bond ladder | Income; stability ballast | Hold; roll at maturity |
| COP cash-generating investments | Currency hedging; COP income | BBVA/Bancolombia CDTs, TES bonds — managed via `scripts/07_evaluate_colombia_investments.R` |

---

## DEPLOYMENT STRATEGY: $5,200/MONTH

### Monthly Investment Plan
Using the $5,188/month excess cash flow (conservative estimate):

#### Phase 1: Immediate (Months 1–3)
Deploy existing $309k liquid reserve strategically:
- **$100k → Wealthfront Cash Account** (maintain emergency fund; earns competitive APY automatically)
- **$50k → Bond ladder extension** (extend existing position at maturity)
- **$159k → Wealthfront Joint Investment Account** (single transfer; Wealthfront deploys automatically per risk score)

**Month 1 end: Core portfolio funded and actively managed**

#### Phase 2: Monthly Automated Investing (Months 4+)
- **Set up $5,200/month automatic deposit → Wealthfront Joint Investment Account**
- Wealthfront allocates and invests automatically; no manual ETF purchases needed
- Tax-loss harvesting runs daily in background

**Total monthly:** $5,200 (within available cash flow)

#### Currency Diversification (Ongoing)
When USD/COP exposure gap closes to target (≤5% drift from 80/20), redirect a portion of monthly surplus to COP instruments:
- CDTs at BBVA or Bancolombia (Fogafin-insured, 9.5–10.5% nominal yield)
- TES government bonds (10–12% nominal) via Colombian brokerage
- Track and evaluate monthly using `scripts/08_quarterly_dual_currency_review.R`

---

## GROWTH PROJECTIONS

### Base Case: 7.5% Annual Return, $5,200/Month Contribution

| Year | Annual Contribution | Investment Gain | Year-End Balance |
|------|-------------------|-----------------|-----------------|
| 0 | — | — | $557,994 |
| 1 | $62,400 | $44,600 | $664,994 |
| 2 | $62,400 | $50,250 | $777,644 |
| 3 | $62,400 | $58,323 | $898,367 |
| 4 | $62,400 | $67,378 | $1,028,145 |
| 5 | $62,400 | $77,611 | $1,168,156 |
| 10 | $62,400/yr | $142,400/yr | $1,759,200 |

**Key Insight:** Even with moderate 7.5% returns, net worth exceeds $1M by year 5 and approaches $1.8M by year 10.

### Scenario Analysis

| Scenario | Annual Return | Year 5 Balance | Year 10 Balance |
|----------|----------------|---------|---------|
| **Conservative** | 5.0% | $1,031,000 | $1,458,000 |
| **Base Case** | 7.5% | $1,168,000 | $1,759,000 |
| **Optimistic** | 9.0% | $1,247,000 | $2,011,000 |

---

## ACTION ITEMS: IMMEDIATE (NEXT 30 DAYS)

### 1. Configure Wealthfront Accounts
- [ ] Log into Wealthfront; verify Individual Cash Account ($153k) is active
- [ ] Open or fund **Wealthfront Investment Account** (taxable; individual or joint)
- [ ] Set risk score to **7.5–8.5** based on guaranteed income floor and long horizon
- [ ] Enable **tax-loss harvesting** (should be on by default; confirm)

### 2. Initial Deployment ($209,908)
- [ ] Keep $100,000 in Wealthfront Cash Account (emergency fund; earns APY automatically)
- [ ] Transfer $50,000 → extend existing bond ladder (maturity schedule permitting)
- [ ] Transfer $159,908 → Wealthfront Investment Account (single transfer; auto-deployed)

### 3. Set Up Automated Monthly Investing
- [ ] Configure **$5,200/month automatic deposit** to Wealthfront Investment Account
- [ ] Set deposit date to 1st of each month
- [ ] No further manual action needed — Wealthfront invests and rebalances automatically

### 4. Tax Planning
- [ ] Review IRA contribution room for 2026 ($7,500 limit at age 64)
- [ ] Note: Wealthfront tax-loss harvesting handles day-to-day tax optimization automatically
- [ ] Consult CPA annually for overall tax strategy (qualified dividends, IRA timing, etc.)

### 5. Ongoing Monitoring
- [ ] **Quarterly:** Run `source("scripts/04_quarterly_wealth_dashboard.R")` + `source("scripts/08_quarterly_dual_currency_review.R")`
- [ ] **Annual:** Review Wealthfront risk score if circumstances change (income, health, goals)
- [ ] **Annual goal check:** Progress toward $500k estate ✓ (already achieved)

---

## FALSIFICATION TRIGGERS

If any of the following occur, reassess the plan:

1. **Guaranteed income drops below $5,000/month** → Reduce equity exposure
2. **Major healthcare costs exceed $50,000/year** → Increase cash reserve
3. **Market downturn >30% two years running** → Review allocation for risk tolerance
4. **Spending increases to $4,000+/month** → Reduce investment rate
5. **Interest rates spike to 6%+** → Reconsider bond allocation

---

## CONCLUSION & RECOMMENDATION

**Your financial position is strong.** You have:
- ✅ Exceeded your primary $500k estate goal
- ✅ Guaranteed income that covers expenses 3.25x over
- ✅ Massive liquid reserves ($310k) enabling aggressive investment
- ✅ Long investment horizon (20–30+ years)
- ✅ Ability to sustain 50% equity allocation despite age/disability

**Immediate priority:** Deploy excess cash into diversified portfolio generating 5–8% returns. At this rate, your estate will exceed $1M by year 5 and $1.8M+ by year 10—far exceeding the $500k target.

**Next step:** Execute Phase 1 deployment in next 30 days. Dollar-cost average the remaining $209k, then commit to $5,200/month systematic investment.

---

## DISCLAIMER

This is research and analysis only, not personalized financial advice. Consult a qualified financial advisor before making investment decisions. Tax implications, insurance needs, and long-term care planning are beyond the scope of this analysis.

---

**Document prepared:** September 21, 2026  
**Data source:** Banktivity QIF Export (2026-09-21_fullBanktivity.qif)  
**Next review:** December 31, 2026
